...
model = load_model('./save/model_test01_1.h5') #오류 Use `model.compile(optimizer, loss)`
model = load_model('./save/model_test01_2.h5') 
#컴파일 이후에 저장된 모델을 불러오면 아래의 '3. 컴파일, 훈련'이 제거되도 이전에 훈련했던 가중치까지 저장하기때문에 save했을때와 결과값이 같음
model.summary()
